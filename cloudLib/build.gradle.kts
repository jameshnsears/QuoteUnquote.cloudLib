    ...
    productFlavors.forEach { productFlavorName ->
        buildTypes.forEach { buildTypeName ->
            ...
            tasks.register<JacocoReport>("test${capitalizedVariantName}Coverage") {
                ...
                val kotlinClasses =
                    fileTree("$buildDir/intermediates/kotlin_classes/$variantName/classes") {
                        setExcludes(fileExclusions)
                    }
                val javaClasses =
                    fileTree("$buildDir/intermediates/javac/$variantName/classes") {
                        setExcludes(fileExclusions)
                    }
                val kotlinClassesFallback =
                    fileTree(
                        "$buildDir/intermediates/built_in_kotlinc/$variantName/compile${capitalizedVariantName}Kotlin/classes",
                    ) {
                        setExcludes(fileExclusions)
                    }
                val javaClassesFallback =
                    fileTree(
                        "$buildDir/intermediates/javac/$variantName/compile${capitalizedVariantName}JavaWithJavac/classes",
                    ) {
                        setExcludes(fileExclusions)
                    }

                classDirectories.setFrom(
                    files(kotlinClasses, javaClasses, kotlinClassesFallback, javaClassesFallback),
                )
                sourceDirectories.setFrom(
                    files(
                        "$projectDir/src/main/java",
                        "$projectDir/src/main/kotlin",
                        "$projectDir/src/$productFlavorName/java",
                        "$projectDir/src/$productFlavorName/kotlin",
                    ),
                )
                executionData.setFrom(
                    fileTree(buildDir) {
                        include("outputs/unit_test_code_coverage/${variantName}UnitTest/*.exec")
                    },
                )
                ...
                reports {
                    ...
                    xml.outputLocation.set(file("$buildDir/reports/jacoco/${variantName}_unit.xml"))
                    html.outputLocation.set(file("$buildDir/reports/jacoco/${variantName}_unit"))
                }
            }
        }
    }
